...
class TableExtractor:
    ...
    def detect_table_region(self, image: np.ndarray) -> Tuple[bool, np.ndarray, List[List[TableCell]]]:
        """
    ...
    def process_multi_page_table(self, images: List[np.ndarray]) -> List[Dict[str, Any]]:
        ...
        for page_num, image in enumerate(images):
            ...
            table_found, table_region, cells = self.detect_table_region(image)
            if not table_found:
                continue
            ...
            if header_structure:
                page_data = self._extract_table_data_from_cells(cells, header_structure)
                all_table_data.extend(page_data)
        
        return all_table_data

    def validate_table_data(self, table_data: List[Dict[str, Any]]) -> TableValidation:
        """
    ...
    def _extract_table_data_from_cells(self, cells: List[List[TableCell]], 
    ...
    def extract_table_data(self, image: np.ndarray) -> List[Dict[str, Any]]:
        ...
        table_data = self._extract_table_data_from_cells(cells, header_structure)
        
        # Validate extracted data
        validation = self.validate_table_data(table_data)
    ...
    def detect_table_structure(self, image: np.ndarray) -> List[Dict[str, int]]:
        ...
        for header in header_words:
            col_type = self._map_header_to_column(header['text'])
            if col_type:
                columns.append({
                    'type': col_type,
                    'x': header['x'],
    ...
    def _map_header_to_column(self, header_text: str) -> str:
